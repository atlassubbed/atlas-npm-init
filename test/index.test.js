...
describe("npm starter package generator", function(){
  ...
  it("should create a nameless project in the dest folder", function(done){
    buildAndCheck({d: "my desc"}, "npm-package", (err, info) => {
      if (err) return done(err);
      expect(info.isDirectory()).to.be.true;
      done()
    })
  ...
  it("should make an initial commit in the project if author has name and email", function(done){
    ...
    buildAndRun({n, a}, "git log", (err, out) => {
      if (err) return done(err);
      expect(out).to.have.string("Adds boilerplate src/test files, etc. Commit made by atlas-npm-init, an npm starter-app generator.")
      done()
    })
  })
  ...
  it("should populate the project's package.json correctly", function(done){
    const n = "my-project", d = "my desc", a = "atlassubbed"
    buildAndRead({n,d,a}, join(n, "package.json"), (err, data) => {
      if (err) return done(err);
